...
public class DT2vsDT2C {
            ...
            for (int gap = randInt(1, length); gap >= 1; gap = randInt(1, gaps.get(gaps.size() - 1))) {
                ...
                if (gaps.get(gaps.size() - 1) >= 2048) {
                    ...
                    while (left != right) {
                        ...
                        if (avDepth < level + 0.1) right = mid;
                        else left = mid + 1;
                        mid = (left + right) / 2;
                    }
                    gapL = mid - 1;
                    ...
                    while (left != right) {
                        ...
                        avDepth /= depLen;
                        //System.err.println("Gap: " + mid + " Depth: " + avDepth);
                        if (avDepth > level - 0.1) left = mid + 1;
                        else right = mid;
                        mid = (left + right) / 2;
                    }
                    gapR = mid + 1;
    ...
    public static void main(String[] args) {
        ...
        for (; ; seed++, counter.incrementAndGet()) {
            args = new String[]{
                "--length", "10000",
                "--testtry", "100",
            ...
            ArrayList<Integer> gaps1 = DepthTargeted2.getGaps(args);
            ArrayList<Integer> gaps2 = DepthTargeted2C.getGaps(args);
            if (gaps1.equals(gaps2)) {
                System.out.println("SEED " + seed + " PRODUCES THE SAME GAP SEQUENCE.");
                System.out.println(gaps1);
                System.out.println();
            } else {
                System.out.println("After testing " + counter.get() + " seeds...");
                System.out.println("SEED " + seed + " PRODUCES DIFFERENT GAP SEQUENCES.");
                System.out.println("DT2: " + gaps1);
                System.out.println("DT2C: " + gaps2);
                return;
            }
            if (seed == Long.MAX_VALUE) return;
        }
    }
}